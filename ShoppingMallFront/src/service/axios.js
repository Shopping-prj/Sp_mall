...
api.interceptors.request.use(
  async (config) => {
    ...
    if (isTokenExpiringSoon(token)) {
      console.log("⏳ accessToken 만료 임박 → refresh 시도");
      ...
      if (refreshToken) {
        try {
          ...
          console.error("❌ 선제적 refresh 실패", err);
          const ctx = getAuthContextRef();
...
api.interceptors.response.use(
  (res) => {
    console.log("📌 [Response 성공]", res.status, res.config.url);
    return res;
  },
  async (error) => {
    console.log("📌 [Response 에러 감지]", error.response?.status, error.config?.url);
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        console.log("📌 [Refresh 시도] refreshToken:", refreshToken);
        ...
        if (ctx?.setToken) {
          ...
          console.log("📌 [AuthContext 갱신] 새 토큰 반영");
        }
