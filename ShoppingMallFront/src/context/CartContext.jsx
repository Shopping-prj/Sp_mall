...
export const CartProvider = ({ children }) => {
  ...
  // :흰색_확인_표시: 최초 로딩: 로그인 사용자만 DB에서 장바구니 로드
  useEffect(() => {
    if (isLoggedIn) {
      getCartByEmail() // email은 토큰 기반으로 백엔드에서 추출
        ...
        .catch((err) => {
          console.error(":x: 장바구니 불러오기 실패:", err);
        });
    } else {
      setCartItems([]); // 로그인 안 된 상태 → 비워둠
    }
  }, [isLoggedIn, cartKey]);
  // :흰색_확인_표시: 로그인 여부 체크 공통 함수
  const requireLogin = () => {
    if (!isLoggedIn) {
      alert("로그인이 필요합니다.");
      navigate("/login");
  ...
  const handleAddToCart = async (product, count = 1) => {
    ...
    try {
      ...
      console.error(":x: addToCart 실패:", err);
    }
  };
  // :흰색_확인_표시: 단일 삭제
  const handleRemoveItem = async (ci_no) => {
    ...
    try {
      ...
      console.error(":x: removeCartItem 실패:", err);
    }
  };
  // :흰색_확인_표시: 전체 삭제
  const handleClearCart = async () => {
    ...
    try {
      ...
      console.error(":x: clearCart 실패:", err);
    }
  };
  // :흰색_확인_표시: 수량 변경
  const handleChangeCount = async (item, delta) => {
    ...
    try {
      ...
      console.error(":x: changeCount 실패:", err);
    }
  };
  const handleOrder = () => {
    if (!requireLogin()) return;
