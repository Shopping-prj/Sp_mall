...
export default function OrderShipping() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      setRows(list);
      setChecked(new Set());
      if (list.length === 0) setAlert({type:"secondary", msg:"배송중 주문이 없습니다."});
    } catch (e) {
      console.error(e);
  ...
  const saveShipment = async () => {
    ...
    if (missing.length) { setAlert({type:"warning", msg:`택배사/송장 누락 ${missing.length}건`}); return; }
    ...
    try {
      ...
      setAlert({type:"success", msg:"송장 정보를 저장했습니다."});
    } catch {
  ...
  const bulkStatus = async (next) => {
    ...
    try {
      ...
      if (!rs.every(r => r.ok)) throw new Error();
      await fetchList();
      setAlert({type:"success", msg:`선택 주문을 ‘${next}’ 처리했습니다.`});
    } catch {
      setAlert({type:"danger", msg:"상태 변경 실패"});
    } finally { setLoading(false); }
  };
    ...
    <div className="container-fluid py-3">
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <button className="btn btn-outline-primary btn-sm" onClick={()=>bulkStatus("배송완료")} disabled={loading}>선택 배송완료</button>
          <button className="btn btn-outline-danger btn-sm" onClick={()=>bulkStatus("취소")} disabled={loading}>선택 취소</button>
      ...
      <div className="table-responsive">
        <table className="table table-bordered table-hover align-middle">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={11} className="text-center text-muted py-4">배송중 주문이 없습니다.</td></tr>
            ) : (
              filtered.slice(0, pageSize).map(o => {
                ...
                return (
                  <tr key={id}>
                    ...
                    <td>
                      <input className="form-control form-control-sm" placeholder="송장번호 입력" value={o.invoiceNo || ""} onChange={(e)=>setRowField(id, "invoiceNo", e.target.value.replace(/[^\d-]/g,""))} />
                    </td>
                    <td className="text-center">{o.mpOrder || "배송중"}</td>
