...
export default function OrderWaiting() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      // ✅ 여기서 먼저 ok 확인
      if (!res.ok) {
        const text = await res.text();  // JSON 아님 → 그냥 텍스트 받기
      ...
      const mapped = (Array.isArray(data) ? data : data.items || []).map((d) => ({
        oNo: d.o_no,
  ...
  const bulkUpdateStatus = async (nextStatus) => {
    ...
    setMsg("");
    try {
      const tasks = [...checked].map((oNo) =>
        fetch(`/api/admin/orders/${oNo}/status`, {
          method: "PUT",
                ...
                return (
                  <tr key={id}>
                    ...
                    <td className="text-end">
                      {Number(r.oAmount || 0).toLocaleString()}
                    </td>
                    <td className="text-center">
                      {statusMap[r.payStatus] || "-"}
                    </td>
                  </tr>
                );
              })
